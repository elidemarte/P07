...
public class Task implements Comparable<Task> {
  ...
  public boolean equals(Object other) {
    ...
    if (((Task) other).getTitle().equalsIgnoreCase(this.title)){
      return true;
    }
    return false;
  }
  ...
  public int compareTo(Task anotherTask) {
    // TODO check this over but think it works

    if (sortingOrder == SortingOrder.TITLE) {
      return title.compareTo(anotherTask.title); // think this works? compareTo seems to match
      // the intended functionality of this method when sorted by Title
    } else if (sortingOrder == SortingOrder.PRIORITY) {
      if (priorityLevel == anotherTask.priorityLevel) {
        return 0;
      } else if (priorityLevel > anotherTask.priorityLevel) {
        return 1;
      } else {
        return -1;
      }
    } else {
      System.out.println("The way you are checking the sortingOrder enums isn't correct.");
      return 0;
    }
  }
